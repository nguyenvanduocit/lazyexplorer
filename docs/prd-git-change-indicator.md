...
| D12 | Màu | badge: `?`/`A` xanh lá (`colGitNew`), `M`/`R` vàng (`colWarn`), `D`/conflict đỏ (`colDanger`); delta **mờ** (`colDim`); folder ● `colDim` | badge mang loại thay đổi → giữ màu trạng thái làm điểm nhấn glance; delta là phụ → `colDim` để lùi về, list pane đọc nhẹ. Dấu `+/-` vẫn phân biệt thêm/bớt nên không cần màu (D3 "đọc được khi mất màu" vẫn giữ). *(Quyết định đổi delta từ xanh/đỏ sang mờ: `adr-git-indicator-perf-display.md` A5.)* |
| D13 | Untracked count cache | `untrackedCache map[path]{mtime,size,lines,ok}` thread qua msg/cmd; cache hit (mtime+size khớp) tái dùng count, **không** đọc lại file | Bỏ đọc `maxPreviewBytes`/file mỗi tick cho file untracked không đổi → idle I/O ~0. Race-free: goroutine chỉ đọc `prev`, model chỉ reassign. Chi tiết + đánh đổi (`git status -uall` walk vẫn là idle floor): `adr-git-indicator-perf-display.md` |
...
3. Với mỗi path **untracked** (`??`, giờ là từng file nhờ `-uall`) là text & ≤ `maxPreviewBytes`:
   ...
   **Cache (D13):** count nhớ theo `(path,mtime,size)`; refresh sau, file không đổi → tái dùng
   count chỉ qua `stat`, **không** đọc lại; `maxUntrackedScan` cap theo **read thật** nên cache
   hit không tốn budget. (`git status -uall` vẫn walk mọi untracked file — idle floor, xem ADR.)
...
gitUntrackedCache untrackedCache // D13: count untracked nhớ giữa các refresh; chỉ pass vào cmd + reassign lúc apply (race-free)
```

**Msg + Cmd:**

```go
type gitRefreshedMsg struct { gen uint64; state gitState; cache untrackedCache }

func gitRefreshCmd(repoRoot string, gen uint64, prev untrackedCache) tea.Cmd {
    return func() tea.Msg {
        state, cache := collectGitState(repoRoot, prev) // goroutine chỉ ĐỌC prev, build cache mới
...
- `Init`: detect repoRoot (đồng bộ, một lần, rẻ); return `tea.Batch(tickCmd(), <git first refresh
  nếu repo>)`. Repo → `gitInFlight=true`, `gitRefreshCmd(repoRoot, gitGen, gitUntrackedCache)` với `gitGen++`.
- `tickMsg` (chỗ `cmd = tickCmd()`): nếu `git.repoRoot != ""` && `!gitInFlight` →
  `gitInFlight=true`, `gitGen++`, `cmd = tea.Batch(tickCmd(), gitRefreshCmd(git.repoRoot, gitGen, gitUntrackedCache))`.
  Ngược lại giữ `cmd = tickCmd()`. (Refresh **độc lập** `dirSig` — D9.)

**Apply (`gitRefreshedMsg`):** `gitInFlight=false`; nếu `msg.gen != gitGen` → drop (stale,
giống discard render cũ); else lưu `m.git = msg.state` **và** `m.gitUntrackedCache = msg.cache`
(swap nguyên khối cùng gen-gate — state + cache nhất quán, không mutate in-place → race-free, D13).
View kế tiếp (poll tick luôn trigger re-render mỗi 1s) tự phản ánh map mới — không cần đụng
`dirSig`/`fsSig`.
...
- **file inactive**: name `fileStyle` + badge `color` + (space) + delta **mờ** (`dimStyle`),
  gap trống. Badge giữ màu trạng thái (điểm nhấn glance); delta lùi về `colDim`.
...
```go
colGitNew = lipgloss.Color("#3FB950") // new/untracked/added badge — xanh git (github green)
// badge Foreground theo gitCode→màu; folder ● và delta ("+N"/"-N") đều dùng dimStyle (colDim).
```
