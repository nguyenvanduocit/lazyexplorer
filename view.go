...
// place a row is formatted, so list pane and folder preview can never disagree
// on row format — see docs/prd-consistent-file-listing.md §5.1. active=true
// marks the cursor row in the list pane (caret + full-width highlight). Folder
// preview always passes active=false.
//
...
func renderEntryRow(e entry, w int, active, listFocused bool) string {
	...
	if active {
		...
		return st.Width(w).Render("▶ " + fitRow(name, size, w-2))
	}
	if e.isDir {
